...
double Cargo_Car::capacity = 1000;
...
void* Cargo_Car::cargo_car_thread(void* arg) {
    ...
    cargo_car->state = Cargo_Car::Idle;
    while (cargo_car->is_running) {
        ...
        required = capacity;
        while (required > 0) {
            sprintf(cargo_car->status, "Resupplying detergent, %4d left",
                    required);
            if (required < 50) {
        ...
        sprintf(cargo_car->status, "Idle");
        cargo_car->state = Idle;
        cargo_car->target = nullptr;
        cargo_car->cargo_cycles++;
    }
}
char* Cargo_Car::getStatus() const { return status; }


uint32_t Cargo_Car::getCargoCycles() const { return cargo_cycles/2; }
